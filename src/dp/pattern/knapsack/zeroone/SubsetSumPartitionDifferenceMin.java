...
class SubsetSumPartitionDifferenceMin {
	...
	private int minDifferencePartitionT(int[] items, int n) {
		...
		/*-
		       s1 + s2  = maxSum......(i)
		    => s2 = maxSum - s1  // Now we need to look for only one subset
		*/
	...
	public static void main(String[] args) {
		...
		String sb = "Minimum difference possible between two subsets \nRecursive : "
				+ minDifferencePartition.minDifferencePartitionR(items, n) + "\n" + "Memoized (Top-Down) : "
				+ minDifferencePartition.minDifferencePartitionM(items, n) + "\n" + "Tabulation (Bottom-Up) : "
				+ minDifferencePartition.minDifferencePartitionT(items, n);
